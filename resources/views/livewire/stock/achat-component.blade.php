<div>
    ...
    <div class="col-md-12">
        <div class="card card-primary card-outline">
                                ...
                                @foreach ($achats as $entree)
                                    ...
                                    @else
                                        <tr>
                                            ...
                                            <td>{{ $entree->user->name }}</td>
                                            <td class="text-center">
                                                <button wire:click='show({{ $entree->id }})' data-toggle="modal" data-target="#AchatModal" class="btn btn-link btn-sm" type="button"><i class="fas fa-plus-circle"></i></button>
                                                <button wire:click='showOrder({{ $entree->id }})' data-toggle="modal" data-target="#showAchatModal" class="btn btn-link btn-sm" type="button"><i class="fa fa-eye text-secondary" aria-hidden="true"></i></button>
                                                <button wire:click='showDeleteachatDialog({{ $entree->id }})' class="btn btn-link btn-sm" type="button"><i class="fas fa-trash text-danger"></i></button>
                                                <a target="_blank" href="" class="btn-link"><i class="fa fa-print" aria-hidden="true"></i></a>
                                            </td>
                                        </tr>
                                    @endif
          ...
          @include('livewire.pharmacie.achats.add-product-to-achat')
          @include('livewire.pharmacie.achats.show-achat')
        </div>
    </div>
