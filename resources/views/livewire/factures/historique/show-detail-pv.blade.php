...
<!-- Modal -->
<div wire:ignore.self class="modal fade" id="showdDetailHistikPvrModal"
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                           ...
                           <table class="table table-sm">
                               ...
                               <tbody>
                                   @if ($demande->consultation->price_prive== 0 )
                                   ...
                                   <div>
                                    @if ($demande->is_valided==true)
                                        <tr class="text-success">
                                            ...
                                            <td class="text-right">{{number_format($demande->consultation->price_prive*$valeur_taux,1)}}</td>
                                            <td class="text-right">{{number_format($demande->consultation->price_prive*1*$valeur_taux,1)}}</td>
                                    ...
                                    @endif
                                        ...
                                        @php
                                            $total_cons=+$demande->consultation->price_prive*1*$valeur_taux;
                                        @endphp
                                        <tr>
                                            <td></td>
                                            <td></td>
                                            <td></td>
                                            <td></td>
                                            <td class="border border-dark text-right p-2 text-danger text-bold">Total: {{number_format($demande->consultation->price_prive*1*$valeur_taux,1)}}</td>
                                        </tr>
                                </div>
                                   @endif
                                    <div>
                        ...
                        <div class="card-body">
                            <div class="d-flex justify-content-between">
                                <div class="p-2">
                                    ...
                                    @if ($demande->reduction==null)
                                        @if ($isReduction==true)
                                            <div class="form-group mt-2">
                                                <label for="">Montant</label>
                                                <input wire:keydown.enter='reducted({{$demande->id}})' wire:model.defer='mt_reduction'
                                                    placeholder="Montant reductio ici..." class="form-control" type="text">
                                            </div>
                                        @else
                                            <button wire:click='isReduction({{$demande->id}})' class="btn btn-primary btn-sm" type="button">Reduction</button>
                                        @endif
                                    @else
                                        <button wire:click='deleteReduction({{$demande->id}})' class="btn btn-danger btn-sm" type="button">Annuler Reduction</button>
                                    @endif

                                    @if ($demande->accompte==null)
                                        @if ($isAccompte==true)
                                            <div class="form-group mt-2">
                                                <label for="">Montant</label>
                                                <input wire:keydown.enter='accompted({{$demande->id}})' wire:model.defer='mt_accompte'
                                                    placeholder="Montant accompte ici..." class="form-control" type="text">
                                            </div>
                                        @else
                                            <button wire:click='isAccompte({{$demande->id}})' class="btn btn-secondary btn-sm" type="button">Accompte</button>
                                        @endif
                                    @else
                                        <button wire:click='deleteAccompte({{$demande->id}})' class="btn btn-danger btn-sm" type="button">Annuler Accompte</button>
                                    @endif

                                    @if ($demande->is_valided==false)
                                        <button wire:click='marquerFini({{$demande->id}})' class="btn btn-success btn-sm" type="button">Marquer fini</button>
                                    @else
                                        <button wire:click='annulerMarquerFini({{$demande->id}})' class="btn btn-success btn-sm" type="button">Annuler fini</button>
                                    @endif
                                </div>
                                <div></div>
                                <div>
                                    @if ($demande->reduction!=null)
                                        ...
                                        <div class="d-flex justify-content-between p-2 border border-dark text-bold text-primary" style="font-size: 18px">
                                            <div class="mr-4 ">Reduction:</div>
                                            <div></div>
                                            <div class="ml-4">{{number_format($demande->reduction->mt,1)}} Fc</div>
                                        </div>
                                        <div class="d-flex justify-content-between p-2 border border-dark text-bold text-primary" style="font-size: 18px">
                                            <div class="mr-4 ">Reste à payer:</div>
                                            <div></div>
                                            <div class="ml-4">{{number_format($total_general-$demande->reduction->mt,1)}} Fc</div>
                                        </div>
                                    @elseif ($demande->accompte!=null)
                                        <div class="d-flex justify-content-between p-2 border border-dark text-bold text-primary" style="font-size: 18px">
                                            <div class="mr-4 ">Total général:</div>
                                            <div></div>
                                            <div class="ml-4">{{number_format($total_general,1)}} Fc</div>
                                        ...
                                        <div class="d-flex justify-content-between p-2 border border-dark text-bold text-primary" style="font-size: 18px">
                                            <div class="mr-4 ">Accompte:</div>
                                            <div></div>
                                            <div class="ml-4">{{number_format($demande->accompte->mt,1)}} Fc</div>
                                        </div>
                                        @if ($demande->accompte->mt>$total_general)
                                            <div class="d-flex justify-content-between p-2 border border-dark text-bold text-primary" style="font-size: 18px">
                                                <div class="mr-4 ">Reste à payer:</div>
                                                <div></div>
                                                <div class="ml-4">{{number_format(0,1)}} Fc</div>
                                            </div>
                                            <div class="d-flex justify-content-between p-2 border border-dark text-bold text-primary" style="font-size: 18px">
                                                <div class="mr-4 ">Remise:</div>
                                                <div></div>
                                                <div class="ml-4">{{number_format($demande->accompte->mt-$total_general,1)}} Fc</div>
                                            </div>
                                        @else
                                            <div class="d-flex justify-content-between p-2 border border-dark text-bold text-primary" style="font-size: 18px">
                                                <div class="mr-4 ">Reste à payer:</div>
                                                <div></div>
                                                <div class="ml-4">{{number_format($total_general-$demande->accompte->mt,1)}} Fc</div>
                                            </div>
                                        @endif
                                    @else
                                        <div class="d-flex justify-content-between p-2 border border-dark text-bold text-primary" style="font-size: 18px">
