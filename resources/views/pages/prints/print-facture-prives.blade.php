  ...
  <body>
    ...
    <div class="entete">
        <div class="gauche">
            ...
            <div><Span><strong>Type </strong> {{ $demande->fiche->type }}/Ambulant</Span><br></div>

            @else
            <div><Span><strong>Type </strong> {{ $demande->fiche->type }}/Hospitalisé</Span><br></div>
            @endif
    ...
    <div class="contenu">
        ...
        @endphp
         ...
         @if($demande->consultation->price_prive==0 and $demande->consultation->price_prive==0)
               ...
               <thead>
                ...
                <tbody>
                        <tr>
                            ...
                            <td class="m-price2" style="">{{ number_format($demande->consultation->price_prive*$valeur_taux) }} CDF</td>
                        </tr>
                        <tr class="row">
                            <td ></td>
                            <td class="m-price" style=" ">Total: {{ number_format($demande->consultation->price_prive*$valeur_taux,2) }} CDF</td>
                            {{ $total_cons+=$total_cons+$demande->consultation->price_prive*$valeur_taux }}
                        </tr>
                </tbody>
            </table>
         @endif
                ...
                <tbody>
                    ...
                    @foreach ($demande->medications as $medication)
                        <tr>
                            ...
                            <td class="m-price">{{number_format($medication->product->price,1)}}</td>
                            <td class="m-price">{{number_format($medication->product->price*$medication->qty)}}</td>
                        </tr>
                        @php
                    ...
                    @endforeach
                        ...
                        <tr class="row">
                            ...
                            <td>Total: </td>
                            <td class="m-price">
                                {{  number_format($total_medication) }} Fc
                            </td>
                        </tr>
                </tbody>
            </table>
            ...
            @endif
         @php
            $net_a_payer=$total_cons+$total_labo+$total_radio+
            $total_medication+$total_chirurgie+$total_autres+$total_oreille
            +$total_dentisterie+$total_sejours+$total_echo+$total_gyneco+$total_nursing
        @endphp
        <div style="text-align: right;margin-top: 10px;">
            @if ($demande->reduction!=null)
                <div style="border: 1px solid black; padding: 10px">
                    <span style="font-size: 15px;font-weight: bold">Total à payer: {{ number_format($net_a_payer,2) }} Fc</span>
                </div>
                <div style="border: 1px solid black; padding: 10px">
                    <span style="font-size: 15px;font-weight: bold">Reduction: {{ number_format($demande->reduction->mt,2) }} Fc</span>
                </div>
                <div style="border: 1px solid black; padding: 10px">
                    <span style="font-size: 15px;font-weight: bold">Reste à payer: {{ number_format($net_a_payer-$demande->reduction->mt,2) }} Fc</span>
                </div>
            @elseif ($demande->accompte!=null)
                ...
                <div style="border: 1px solid black; padding: 10px">
                    <span style="font-size: 15px;font-weight: bold">Accompte: {{ number_format($demande->accompte->mt,2) }} Fc</span>
                </div>

                @if ($demande->accompte->mt>$net_a_payer)

                @else
                    <div style="border: 1px solid black; padding: 10px">
                        <span style="font-size: 15px;font-weight: bold">Reste: {{ number_format($net_a_payer-$demande->accompte->mt,2) }} Fc</span>
                    </div>
            ...
            @else
                <div style="border: 1px solid black; padding: 10px">
                    <div>
                        <span style="font-size: 15px;font-weight: bold">Total CDF: {{ number_format($net_a_payer,2) }} Fc</span>
                    </div>
                    <div>
                    <span style="font-size: 15px;font-weight: bold">Total USD: {{ number_format($net_a_payer/$valeur_taux,1) }}$ </span>
                    </div>
                </div>
            @endif
