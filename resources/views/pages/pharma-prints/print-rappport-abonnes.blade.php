...
<body>
    ...
    <div style="text-align: center;padding: 15px;font-size: 22px"><span>RAPPORT DES SORTIES DES ABONNES/{{$name_abn}}</span></div>
    <div style="padding: 5px;font-size: 19px;">
        <span><span style="font-weight: bold;text-align: left">Date</span> :{{date('d/m/Y')}} </span><br>
    </div>
    @php
    ...
    <div>
        <table class="">
            ...
            <tbody>
                @foreach ($demande as $key => $dmd)
                    @if ($dmd->products->isEmpty())

                    @else
                    <tr style="font-family:Consolas, Menlo, Monaco, Lucida Console,
                        Liberation Mono, DejaVu Sans Mono, Bitstream Vera Sans Mono,
                        Courier New, monospdmdace, serif">
                        <td style="text-align: center">{{ $key+1 }}</td>
                        <td>{{ $dmd->Nom.' '.$dmd->Prenom }}</td>
                        <td style="padding: 5px">
                            <table>
                                ...
                                <tbody>
                                @foreach ($dmd->products as $product)
                                        <tr>
                                            <td>{{ $product->name }}</td>
        ...
        <div style="text-align: right;font-size: 18px;margin-top: 10px;font-family:Consolas, Menlo, Monaco, Lucida Console,
        Liberation Mono, DejaVu Sans Mono, Bitstream Vera Sans Mono,
