...
class HistoriqueFacturesAbonnesComponent extends Component
{
    use WithPagination;
    ...
    public $abonnements,$abonnementData,$mois=[];
...
//----------- FIN -----------------
    ...
    {
        $abn=Abonnement::find($this->abonnementData);
        if ($abn) {
            $this->speciales=DemandeSpeciale::whereMonth('created_at',$this->currentDate)
            ->where('type',$abn->name)
        ...
        return view('livewire.factures.historique.historique-factures-abonnes-component',
            [
                'factures'=>$data,
                'fac_speciales'=>$this->speciales
            ]
        );
    }
}
